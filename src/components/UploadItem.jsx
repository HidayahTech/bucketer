...
export function UploadItem({ item, onResume, onRestart, onCancel, onRemove, onDismissLargeWarn, provider }) {
  const { name, size, status, bytesUploaded, speed, error, expiryWarning, resumeRecord, largeFileWarningDismissed } =
    item;
  ...
  return (
    <div class="upload-item">
      <div class="upload-item-header">
        <span class="upload-item-name" title={name}>
          {name}
        </span>
        ...
        <span class={`upload-item-status ${statusClass}`}>
          {status === 'uploading' || status === 'resuming' ? (
            <>
              <span class="spinner" style={{ marginRight: '.3rem' }} />
            </>
          ) : null}
          {statusLabel}
        </span>
        <div style={{ display: 'flex', gap: '.3rem' }}>
          ...
          {status === 'paused' && (
            <>
              <button class="btn btn-primary btn-sm" onClick={onResume}>
                Resume
              </button>
              <button class="btn btn-ghost btn-sm" onClick={onRestart}>
                Restart
              </button>
            </>
          )}
          ...
          {(status === 'done' || status === 'error' || status === 'aborted') && (
            <button class="btn btn-ghost btn-sm" onClick={onRemove}>
              ✕
            </button>
          )}
        </div>
      ...
      {showProgress && (
        <>
          ...
          <div class="upload-meta">
            <span>
              {Math.floor(displayedBytes).toLocaleString()} / {size.toLocaleString()} B
            </span>
            {status === 'uploading' && speed > 0 && (
              <span>
                {formatSpeed(speed)} · ETA {formatEta(liveEta)}
              </span>
            )}
      ...
      {size >= LARGE_FILE_WARN && !largeFileWarningDismissed && (
        <div class="banner banner-warn" style={{ marginTop: '.4rem', padding: '.4rem .6rem', fontSize: '.78rem' }}>
          <div class="banner-body">
            ...
            <code>rclone</code>, the B2 CLI, or the AWS CLI offer better reliability (checksumming, bandwidth
            throttling, and resumability outside browser constraints). You can still proceed in-browser, but be aware of
            these limitations.
          </div>
          <button class="banner-close" onClick={() => onDismissLargeWarn()}>
            ✕
          </button>
        </div>
      )}
      ...
      {resumeRecord && status === 'paused' && (
        <div style={{ fontSize: '.78rem', color: 'var(--text-muted)', marginTop: '.3rem' }}>
          Resume record found from {new Date(resumeRecord.startedAt).toLocaleString()}. Re-select the same file to
          resume.
        </div>
      )}
