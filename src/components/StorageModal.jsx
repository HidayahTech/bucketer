      ...
      {!data ? (
        ...
        <div class="storage-modal-body">
          ...
          <details class="sv-section" open>
            ...
            <div class="sv-section-body">
                  ...
                  <tbody>
                    <tr>
                      <td class="sv-key">
                        Endpoint
                        <br />
                        <KeyName name="s3b_endpoint" />
                      </td>
                      <td class="sv-val" title={data.creds.endpoint}>
                        {truncate(data.creds.endpoint, 40) || <span class="sv-nil">—</span>}
                      </td>
                    </tr>
                    <tr>
                      <td class="sv-key">
                    ...
                    <tr>
                      <td class="sv-key">
                        Key ID
                        <br />
                        <KeyName name="s3b_key_id" />
                      </td>
                      <td class="sv-val sv-mono">
                        {data.creds.keyId ? truncate(data.creds.keyId, 12) : <span class="sv-nil">—</span>}
                      </td>
                    </tr>
                    <tr>
                      <td class="sv-key">
                    ...
                    <tr>
                      <td class="sv-key">
                        Region override
                        <br />
                        <KeyName name="s3b_region_override" />
                      </td>
                    ...
                    <tr>
                      <td class="sv-key">
                        Secret key
                        <br />
                        <KeyName name="s3b_secret_key · sessionStorage" />
                      </td>
              ...
              <ConfirmDialog
                ...
                warning={
                  isConnected ? 'You are connected. This will disconnect and reload.' : 'This will reload the page.'
                }
                danger
                reload
              />
          ...
          <details class="sv-section" open>
            ...
            <div class="sv-section-body">
              <StoreLoc>
                localStorage · <KeyName name="s3b_connections" /> (JSON array) · <KeyName name="s3b_credentials" /> (
                {data.credentials.length} stored) · <KeyName name="s3b_last_profile_id" /> (selected)
              </StoreLoc>
              {data.profiles.length === 0 ? (
                ...
                <table class="sv-table">
                  ...
                  <tbody>
                    {data.profiles.map((p) => (
                      <tr key={p.id}>
                        <td class="sv-val" title={p.name}>
                          {truncate(p.name, 24)}
                        </td>
                        <td class="sv-val" title={p.bucket}>
                          {truncate(p.bucket, 20)}
                        </td>
                        <td class="sv-val">{p.provider || <span class="sv-nil">—</span>}</td>
                        <td class="sv-val sv-mono" title={p.keyId}>
                          {truncate(p.keyId, 10)}
                        </td>
                        <td>
                          <button
                            type="button"
                            class="btn btn-ghost btn-sm sv-del-btn"
                            onClick={async () => {
                              deleteConnectionRecord(p.id);
                              await load();
                            }}
              ...
              <ConfirmDialog
                id="profiles"
                controller={controller}
                label="Delete all saved buckets"
          ...
          <details class="sv-section" open>
            ...
            <div class="sv-section-body">
              <StoreLoc>
                IndexedDB: <KeyName name="s3browser" /> (v2) → <KeyName name="bucketer_upload_log" /> · auto-increment
                key
              </StoreLoc>
              {data.log.length === 0 ? (
                    ...
                    <thead>
                      <tr>
                        ...
                        <th>
                          File
                          <br />
                          <KeyName name="fileName" />
                        </th>
                        <th>
                          Destination
                          <br />
                          <KeyName name="destinationKey" />
                        </th>
                        <th>
                          Size
                          <br />
                          <KeyName name="fileSize" />
                        </th>
                        <th>
                          When
                          <br />
                          <KeyName name="completedAt" />
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={i}>
                          <td class="sv-status">
                            {e.status === 'done' ? (
                              ...
                              <span class="sv-cap-denied" title={e.errorMessage}>
                                ✗
                              </span>
                            )}
                          </td>
                          <td class="sv-val" title={e.fileName}>
                            {truncate(e.fileName, 22)}
                          </td>
                          <td class="sv-val sv-muted" title={e.destinationKey}>
                            {truncate(e.destinationKey, 22)}
                          </td>
                      ...
                      {data.log.length > 20 && (
                        <tr>
                          <td colSpan={5} class="sv-more">
                            …and {data.log.length - 20} more
                          </td>
                        </tr>
                      )}
                    </tbody>
          ...
          <details class="sv-section" open>
            ...
            <div class="sv-section-body">
              <StoreLoc>
                IndexedDB: <KeyName name="s3browser" /> (v2) → <KeyName name="s3browser_uploads" /> · key:{' '}
                <KeyName name="provider:endpoint:bucket:destinationKey" />
              </StoreLoc>
                  ...
                  <thead>
                    <tr>
                      ...
                      <th>
                        Started
                        <br />
                        <KeyName name="startedAt" />
                      </th>
                      <th>
                        Part size
                        <br />
                        <KeyName name="partSize" />
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.resume.map((r, i) => (
                      <tr key={i}>
                        <td class="sv-val" title={r.destinationKey}>
                          {truncate(r.destinationKey, 26)}
                        </td>
                        <td class="sv-val" title={r.bucket}>
                          {truncate(r.bucket, 18)}
                        </td>
              ...
              <ConfirmDialog
                id="resume"
                controller={controller}
                label="Discard all resume records"
            ...
            <div class="sv-section-body">
              ...
              <table class="sv-table">
                <tbody>
                  <tr>
                    <td class="sv-key">
                      Max keys per listing
                      <br />
                      <KeyName name="s3b_max_keys" />
                    </td>
                  ...
                  <tr>
                    <td class="sv-key">
                      Part concurrency
                      <br />
                      <KeyName name="s3b_part_concurrency" />
                    </td>
                  ...
                  <tr>
                    <td class="sv-key">
                      File concurrency
                      <br />
                      <KeyName name="s3b_file_concurrency" />
                    </td>
                  ...
                  <tr>
                    <td class="sv-key">
                      Listing cache TTL (s)
                      <br />
                      <KeyName name="s3b_listing_cache_ttl" />
                    </td>
                  ...
                  <tr>
                    <td class="sv-key">
                      ...
                      <br />
                      <KeyName name="s3b_update_check_enabled" />
                    </td>
          ...
          <details class="sv-section" open>
            ...
            <div class="sv-section-body">
              <p class="sv-sub-heading">Capabilities{data.capsConnection ? ` — ${data.capsConnection}` : ''}</p>
              <StoreLoc>
                per connection in <KeyName name="s3b_connections" /> (JSON) · localStorage · values:{' '}
                <KeyName name="'unknown' | 'permitted' | 'denied'" />
              </StoreLoc>
              <table class="sv-table">
                <tbody>
                  {['list', 'download', 'upload', 'delete'].map((op) => (
                    <tr key={op}>
                      <td class="sv-key" style={{ textTransform: 'capitalize' }}>
                        {op}
                      </td>
                      <td class="sv-val">
                        {capIcon(data.caps[op])} {data.caps[op]}
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                {confirmAction === 'caps' ? (
                  <>
                    <span class="sv-confirm-warn">
                      This clears capability state for every saved connection, not just the one shown above.
                    </span>
                    <button type="button" class="btn btn-ghost btn-sm" onClick={() => setConfirm(null)}>
                      Cancel
                    </button>
                    <button type="button" class="btn btn-ghost btn-sm" onClick={() => act('caps')}>
                      Reset all connections
                    </button>
                  </>
                ) : (
                  <button type="button" class="btn btn-ghost btn-sm" onClick={() => setConfirm('caps')}>
                    Reset capabilities
                  </button>
                )}
              </Actions>

              <p class="sv-sub-heading" style={{ marginTop: '.75rem' }}>
                Active uploads tracker
              </p>
              <StoreLoc>
                <KeyName name="s3b_active_uploads" /> (JSON) · localStorage · cross-tab collision detection
              </StoreLoc>
              ...
              <Actions>
                ...
                {confirmAction === 'active' ? (
                  <>
                    ...
                    <button type="button" class="btn btn-ghost btn-sm" onClick={() => act('active')}>
                      Clear tracker
                    </button>
                  </>
                ) : (
                  <button type="button" class="btn btn-ghost btn-sm" onClick={() => setConfirm('active')}>
                    Clear tracker
                  </button>
                )}
              </Actions>
          ...
          <div class="sv-wipe-block">
            ...
            <p class="sv-wipe-desc">Removes every key written to localStorage, sessionStorage, and IndexedDB:</p>
