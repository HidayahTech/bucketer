    ...
    <div class="error-block" role="alert">
      ...
      {isCorsLike && (
        <div style={{ marginTop: '.3rem' }}>
          ...
          {basePrefixUnset &&
            ' A key restricted to a folder inside the bucket can also present this way — if that matches your key, set Base folder in the form above.'}
        </div>
      )}
      {showScopeHint && (
        <div style={{ marginTop: '.3rem' }}>
          <strong>Note:</strong> This can mean the credentials are simply wrong — but it's also common for an access key
          to be restricted to a folder inside the bucket rather than the whole bucket (Backblaze B2 calls this a key's
          Name Prefix). If that matches your key, set <strong>Base folder</strong> in the form above to that folder and
          reconnect.
        </div>
      )}
