...
function collect() {
  const u = [];
  return { onProgress: (x) => u.push({ ...x }), updates: u, done: () => u.find((x) => x.phase === 'done') };
}
...
describe('runMoveOperation against real S3 protocol', () => {
  ...
  test('folder move preserves nested structure under the destination', async () => {
    ...
    await runMoveOperation(
      ctx.client,
      BUCKET,
      { files: [], prefixes: ['photos/2024/'], dest: 'archive/', capturedPrefix: 'photos/' },
      c.onProgress,
    );
  ...
  test('never overwrites: a colliding destination key is skipped, both sides untouched', async () => {
    ...
    await runMoveOperation(
      ctx.client,
      BUCKET,
      { files: [{ key: 'reports/q1.pdf', size: 6 }], prefixes: [], dest: 'archive/', capturedPrefix: 'reports/' },
      c.onProgress,
    );
    const done = c.done();
    assert.equal(done.moved, 0);
    assert.ok(
      done.errors.some((e) => e.skipped),
      'collision is reported as skipped',
    );
  ...
  test('preserves Content-Type and custom metadata across a move (MetadataDirective COPY)', async () => {
    ...
    await runMoveOperation(
      ctx.client,
      BUCKET,
      { files: [{ key: 'a/x.bin', size: 4 }], prefixes: [], dest: 'b/', capturedPrefix: 'a/' },
      () => {},
    );
...
describe('copyObjectMultipart against real S3 protocol', () => {
  test('multipart copy reproduces bytes and carries metadata forward', async () => {
    ...
    await copyObjectMultipart(ctx.client, {
      bucket: BUCKET,
      sourceKey: 'src/big.bin',
      destKey: 'dst/big.bin',
      size: 'multipart-source-bytes'.length,
    });
...
describe('runDeleteOperation against real S3 protocol', () => {
  test('deletes loose files and whole folders', async () => {
    ...
    await runDeleteOperation(
      ctx.client,
      BUCKET,
      { files: ['one.txt'], prefixes: ['junk/'], capturedPrefix: '' },
      c.onProgress,
    );
    const done = c.done();
...
describe('fault injection', () => {
  test('AccessDenied on copy leaves the source in place and reports an error', async () => {
    ...
    await runMoveOperation(
      ctx.client,
      BUCKET,
      { files: [{ key: 'a/x.txt', size: 4 }], prefixes: [], dest: 'b/', capturedPrefix: 'a/' },
      c.onProgress,
    );
