...
describe('matchDownloads — the verdict logic', () => {
  ...
  test('a collision-renamed variant of the right size reads as probably arrived', () => {
    const out = matchDownloads([{ key: 'k/a.txt', localName: 'a.txt', size: 9 }], new Map([['a (1).txt', 9]]));
...
describe('verifyJob — streaming verification with durable outcomes', () => {
  ...
  test('a job can be verified again after a first verification', async () => {
    ...
    const second = await verifyJob(
      'job-1',
      fakeDir([
        { name: 'a.txt', size: 5 },
        { name: 'late.txt', size: 5 },
      ]),
    );
  ...
  test('ambiguous and renamed items stay ISSUED and are never downgraded', async () => {
    ...
    await appendManifestPage(
      'job-1',
      [item('a/x.txt', 5, { localName: 'x.txt' }), item('b/x.txt', 5, { localName: 'x.txt' }), item('r.txt', 9)],
      {},
    );
    ...
    assert.equal(
      await countItemsByStatus('job-1', ITEM_STATUS.ISSUED),
      3,
  ...
  test('SKIPPED (archived) items are never examined or altered', async () => {
    ...
    await appendManifestPage(
      'job-1',
      [item('warm.bin', 5), item('cold.bin', 5, { status: ITEM_STATUS.SKIPPED, skipReason: 'archived' })],
      {},
    );
