...
describe('ConfirmDialog', () => {
  describe('idle state (confirmAction !== id)', () => {
    ...
    test('clicking the trigger calls setConfirm(id)', () => {
      ...
      const { query, cleanup } = mount(<ConfirmDialog id="resume" label="Discard" controller={controller} />);
  ...
  describe('pending state (confirmAction === id)', () => {
    test('renders Cancel + confirm buttons; clicking confirm calls act(id)', () => {
      ...
      const { queryAll, cleanup } = mount(<ConfirmDialog id="log" label="Clear history" controller={controller} />);
      const buttons = queryAll('button');
    ...
    test('explicit warning prop is rendered', () => {
      ...
      const { query, cleanup } = mount(
        <ConfirmDialog
          id="profiles"
          label="Delete all"
          warning="Everything will be removed."
    ...
    test('reload=true with no warning falls back to "This will reload the page."', () => {
      ...
      const { query, cleanup } = mount(<ConfirmDialog id="credentials" label="Clear" reload controller={controller} />);
    ...
    test('no warning and reload=false: no warning span renders', () => {
      ...
      const { query, cleanup } = mount(<ConfirmDialog id="log" label="Clear history" controller={controller} />);
