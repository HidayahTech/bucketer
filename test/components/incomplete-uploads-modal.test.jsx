...
describe('IncompleteUploadsModal', () => {
  test('lists discovered uploads once the scan resolves', async () => {
    ...
    const { text, query, cleanup } = mount(
      h(IncompleteUploadsModal, { scan, discard: async () => {}, onClose: () => {} }),
    );
    await settle();
  ...
  test('shows the empty state when nothing is found', async () => {
    const { query, cleanup } = mount(
      h(IncompleteUploadsModal, { scan: async () => [], discard: async () => {}, onClose: () => {} }),
    );
    await settle();
  ...
  test('Discard calls discard(upload) and removes that row, keeping the others', async () => {
    let discarded = null;
    const scan = async () => [
      { key: 'a', uploadId: 'u1', initiated: null },
      { key: 'b', uploadId: 'u2', initiated: null },
    ];
    const { query, cleanup } = mount(
      h(IncompleteUploadsModal, {
        scan,
        discard: async (u) => {
          discarded = u.uploadId;
        },
        onClose: () => {},
      }),
    );
