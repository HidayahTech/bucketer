...
function inMemoryWorker(captured) {
  ...
  return {
    addEventListener(e, fn) {
      if (e === 'message') listeners.push(fn);
    },
    terminate() {
      captured.bytes = buf;
    },
    async postMessage(m /*, transfer */) {
      if (m.type === 'init') {
        asm = createAssembler(sink, m.layout);
        ...
        try {
          ...
          emit({ type: 'written', key: m.key, crc: r.crc, size: r.size });
        } catch (err) {
          emit({ type: 'entryError', key: m.key, name: err.name, message: err.message });
        }
...
test('runInPlaceJob: all items DONE, finished, valid central directory', async () => {
  ...
  let found = false;
  for (let i = captured.bytes.length - 22; i >= 0 && i > captured.bytes.length - 200; i--) {
...
test('eviction: a staging file that shrinks between runs resets every DONE item back to PENDING', async () => {
  ...
  assert.deepEqual(
    fetchCalls.sort(),
    ['p/a.txt', 'p/b.txt'],
    'both items refetched after eviction, including the one already DONE',
  );
});
...
test('resume guard: DONE items whose zipOffset does not match the key-sorted layout headerOffset are reset to PENDING and rebuilt cleanly (serial completion-order data resumed by in-place)', async () => {
  ...
  await updateItem('j9', 'p/a.txt', {
    ...
    crc: 0,
    time: byKey['p/a.txt'].time,
    date: byKey['p/a.txt'].date,
  });
  await updateItem('j9', 'p/b.txt', {
    ...
    crc: 0,
    time: byKey['p/b.txt'].time,
    date: byKey['p/b.txt'].date,
  });
  ...
  assert.equal(
    res.finished,
    true,
    'the job must still finish — via a clean rebuild, not by finishing on the mismatched offsets',
  );
  assert.deepEqual(
    fetchCalls.sort(),
    ['p/a.txt', 'p/b.txt'],
    'both mismatched-offset DONE items must be reset to PENDING and refetched, not trusted as-is',
  );
...
test('a worker reporting unsupported returns {unsupported:true} immediately, before any fetch, and terminates the worker', async () => {
  ...
  await saveJob(job);
  await appendManifestPage('j4', [{ key: 'p/a.txt', size: 3, lastModified: 0, status: ITEM_STATUS.PENDING }], {
    done: true,
  });
  ...
  const res = await runInPlaceJob(job, {
    ...
    fetchImpl: async () => {
      fetchCalled = true;
      return { ok: true, body: streamOf([1, 2, 3]) };
    },
    root: {},
    concurrency: 2,
  ...
  assert.equal(
    await countItemsByStatus('j4', ITEM_STATUS.PENDING),
    1,
    'items are left untouched for runZipJob to pick up',
  );
});
  ...
  return {
    ...
    async postMessage(m) {
      if (m.type === 'init') {
        ...
        if (m.key === triggerKey) {
          emit({ type: 'fatal', name: 'QuotaExceededError', message: 'quota exceeded' });
          return;
        }
        await asm.writeChunk(m.key, new Uint8Array(m.buffer));
        emit({ type: 'ack', bytes: m.buffer.byteLength });
...
test('a QuotaExceededError fatal from the worker pauses the job (STORAGE block), not a per-item failure, and leaves in-flight items PENDING', async () => {
    ...
    fetchImpl: async (url, { signal } = {}) => {
      ...
      if (key === 'p/a.txt') {
        return {
          ok: true,
          status: 200,
          body: abortAwareTwoChunkBody(new Uint8Array([65, 66]), new Uint8Array([67]), signal),
        };
      }
  ...
  assert.equal(
    await countItemsByStatus('j6', ITEM_STATUS.PENDING),
    2,
    'both items are left PENDING for a resume, not FAILED',
  );
  assert.equal(
    await countItemsByStatus('j6', ITEM_STATUS.FAILED),
    0,
    "a worker fatal is job-wide, not any item's fault",
  );
