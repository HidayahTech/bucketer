...
import { enumerateObjects, groupBySize, deriveSignals, classifyGroups, headSizeGroups } from '../src/lib/dedup-scan.js';
...
describe('classifyGroups', () => {
  test('groups two single-part objects with the same MD5', () => {
    const [g] = classifyGroups([
      [member('new', '2026-02-01', { etagMd5: ETAG_X }), member('old', '2026-01-01', { etagMd5: ETAG_X })],
    ]);
    ...
    assert.deepEqual(
      g.members.map((m) => m.Key),
      ['old', 'new'],
    ); // oldest first (default keeper)
  });

  test('never groups provably-distinct single-part objects (different MD5)', () => {
    const groups = classifyGroups([
      [member('a', '2026-01-01', { etagMd5: ETAG_X }), member('b', '2026-01-02', { etagMd5: ETAG_Y })],
    ]);
    assert.deepEqual(groups, []);
  });
  ...
  test('groups by our stamp when ETag is unavailable', () => {
    const [g] = classifyGroups([
      [member('a', '2026-01-01', { stampHash: STAMP_S }), member('b', '2026-01-02', { stampHash: STAMP_S })],
    ]);
  ...
  test('surfaces same-size multipart objects with no cheap signal as a size candidate', () => {
    const [g] = classifyGroups([
      [member('a', '2026-01-01', { multipart: true }), member('b', '2026-01-02', { multipart: true })],
    ]);
