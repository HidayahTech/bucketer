  ...
  return {
    ...
    send(cmd) {
      ...
      if (name === 'ListObjectsV2Command') {
        ...
        const page = ContinuationToken ? pages.find((p, i) => p.nextToken === ContinuationToken && i > 0) : pages[0];
      ...
      if (multipart) {
        if (name === 'HeadObjectCommand')
          return Promise.resolve({ ContentType: 'application/octet-stream', Metadata: {} });
...
describe('runMoveOperation — folder moves', () => {
  test('discovers, remaps keys under the moved folder, and reports movedPrefixes', async () => {
    ...
    const copyKeys = client.calls
      .filter((c) => c.name === 'CopyObjectCommand')
      .map((c) => c.input.Key)
      .sort();
    ...
    const listPages = new Map([
      [
        ...
        [
          {
            objects: [
              { Key: 'f/ok.txt', Size: 10 },
              { Key: 'f/bad.txt', Size: 10 },
            ],
            isTruncated: false,
          },
        ],
...
describe('runMoveOperation — collisions', () => {
  test('skips an object whose destination key already exists; leaves both sides untouched', async () => {
    ...
    assert.ok(
      !client.calls.some((c) => c.name === 'DeleteObjectCommand'),
      'must not delete the source of a colliding object',
    );
    const done = updates.find((u) => u.phase === 'done');
    assert.equal(done.moved, 0);
  ...
  test('detects intra-batch collisions (two sources mapping to the same destination key)', async () => {
    ...
    const op = {
      files: [
        { key: 'x/dup.txt', size: 10 },
        { key: 'y/dup.txt', size: 10 },
      ],
      prefixes: [],
      dest: 'archive/',
    };
...
describe('runMoveOperation — large objects', () => {
  test('routes objects over 5 GiB through multipart UploadPartCopy, not single copy', async () => {
    ...
    assert.ok(
      client.calls.some((c) => c.name === 'CreateMultipartUploadCommand'),
      'must start a multipart copy',
    );
    assert.ok(
...
describe('runMoveOperation — failures leave data intact', () => {
  ...
  test('delete-denied-after-copy reports a distinct "exists in both places" error and does not count as moved', async () => {
    ...
    assert.ok(
      client.calls.some((c) => c.name === 'CopyObjectCommand'),
      'copy still happens',
    );
    const done = updates.find((u) => u.phase === 'done');
...
describe('runMoveOperation — throttling retry', () => {
  test('retries a throttled copy then succeeds', () =>
    withFastRetry(async () => {
      let copyCalls = 0;
      const client = {
        calls: [],
...
describe('runCopyOperation — copy-and-keep', () => {
  test('copies a loose file and never deletes the source', async () => {
    const client = mockClient({ listPages: emptyDest('archive/') });
    const updates = await copyAndCollect(client, 'bk', {
      files: [{ key: 'docs/a.txt', size: 10 }],
      prefixes: [],
      dest: 'archive/',
    });
    const copies = client.calls.filter((c) => c.name === 'CopyObjectCommand');
    ...
    assert.equal(
      client.calls.filter((c) => c.name === 'DeleteObjectCommand').length,
      0,
      'copy must not delete the source',
    );
    const done = updates.find((u) => u.phase === 'done');
    assert.equal(done.moved, 1);
  });
  ...
  test('renames on collision instead of skipping (never overwrites)', async () => {
    const client = mockClient({
      listPages: new Map([
        ['archive/', [{ objects: [{ Key: 'archive/a.txt' }, { Key: 'archive/a (1).txt' }], isTruncated: false }]],
      ]),
    });
    const updates = await copyAndCollect(client, 'bk', {
      files: [{ key: 'docs/a.txt', size: 5 }],
      prefixes: [],
      dest: 'archive/',
    });
    const copies = client.calls.filter((c) => c.name === 'CopyObjectCommand');
  ...
  test('leaves source rows in place (no movedKeys / movedPrefixes)', async () => {
    const client = mockClient({ listPages: emptyDest('archive/') });
    const updates = await copyAndCollect(client, 'bk', {
      files: [{ key: 'a.txt', size: 1 }],
      prefixes: [],
      dest: 'archive/',
    });
    assert.deepEqual(
      updates.flatMap((u) => u.movedKeys || []),
      [],
      'copy must not signal source-row removal',
    );
  ...
  test('routes large objects through multipart copy, still without deleting', async () => {
    ...
    const client = mockClient({ listPages: emptyDest('archive/'), multipart: true });
    const updates = await copyAndCollect(client, 'bk', {
      files: [{ key: 'big.bin', size: big }],
      prefixes: [],
      dest: 'archive/',
    });
    assert.ok(
      client.calls.some((c) => c.name === 'UploadPartCopyCommand'),
      'large copy uses multipart',
    );
