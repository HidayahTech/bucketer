...
## [1.63.0] — 2026-09-16 — Share a link to the folder you are in

You can now send someone a link that opens Bucketer inside a specific folder (GitLab #69). The header **Copy link** menu offers a labelled choice — **Open in Bucketer — this folder (…/acme/sub/)** first when you are inside a folder, then **Open in Bucketer — top of this bucket** (or *top of this connection* for a base-folder-limited key) — and **Include my access key ID** becomes a checkbox that applies to either link and is remembered for the session. A 🔗 button beside the breadcrumb copies the current folder's link in one click. Toasts say what travelled ("Link copied — opens at clients/acme/sub/"). The recipient still signs in with their own key; the secret is never included. These links are the same family as before with one extra `prefix` parameter: links copied at the root, or at a base folder, are byte-identical to what v1.62.x produced, and a link into a folder the recipient's key cannot reach lands them at their own base folder with a note, never in an error. The per-file link popover now says plainly what it hands out — "Share this file — anyone with the link can open it … treat it like a password" — so the two link kinds can no longer be confused. No per-row folder button: it would crowd mobile rows next to the per-file link, which means something else. README gains a "Sharing links" section, including that a folder name travels with its link (in the URL fragment: never sent to a server, but present in browser history and wherever the link is pasted).
...
layout is known before anything is downloaded, each file is now written straight into its
final place in the archive by a background worker as it arrives, instead of being staged and
copied a second time. Measured about twice as fast on archives of medium-sized files, using
less temporary disk. Everything you see is unchanged — the same single save dialog, progress,
and resume — and browsers without the required worker storage APIs (Safari) automatically use
the previous method. This is a speed improvement, not a memory one: on Firefox, an archive of
very many medium-to-large files can still use significant memory (and this path uses somewhat
more of it than the previous one), so that limitation is unchanged.
